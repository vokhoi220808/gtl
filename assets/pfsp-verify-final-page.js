/* PDF Fusion Smart Pro - Verify System v14 standalone verify page */
(function(){
  'use strict';
  ...
  function entriesFromObject(obj, prefix=''){
    const out=[];
    if(!obj || typeof obj !== 'object') return out;
    Object.entries(obj).forEach(([k,v])=>{
      const label=(prefix ? prefix + ' · ' : '') + k;
      if(Array.isArray(v)){
        if(k === 'customFields') v.forEach(item=>out.push([item.label || 'custom', item.value || '']));
        else if(v.length) out.push([label, v.map(x=>typeof x==='object' ? JSON.stringify(x) : x).join(', ')]);
      }else if(v && typeof v === 'object') out.push(...entriesFromObject(v, label));
      else if(v !== '' && v != null) out.push([label, v]);
    });
    return out;
  }
  function renderDocumentInformation(record={}){
    const info = {documentInfo:record.documentInfo||{}, userInfo:record.userInfo||record.personInfo||{}, extraInfo:record.extraInfo||{}};
    const rows = entriesFromObject(info).filter(x=>String(x[1]||'').trim());
    if(!rows.length) return '';
    return `<div class="pfsp-v14-info"><h3>📄 Document Information / Thông tin tài liệu</h3><div class="pfsp-v14-info-grid">${rows.map(([k,v])=>`<div><b>${html(k)}</b><span>${html(v)}</span></div>`).join('')}</div></div>`;
  }
  function ensureInfoStyles(){
    if(document.getElementById('pfspV14VerifyInfoStyle')) return;
    const st=document.createElement('style'); st.id='pfspV14VerifyInfoStyle'; st.textContent='.pfsp-v14-info{margin-top:14px;border:1px solid rgba(34,211,238,.28);background:rgba(8,47,73,.25);border-radius:18px;padding:14px}.pfsp-v14-info h3{margin:0 0 10px;font-size:16px}.pfsp-v14-info-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.pfsp-v14-info-grid div{border:1px solid rgba(148,163,184,.20);background:rgba(2,6,23,.42);border-radius:13px;padding:10px;min-width:0}.pfsp-v14-info-grid b{display:block;color:#bfdbfe;font-size:11px;text-transform:uppercase;letter-spacing:.04em}.pfsp-v14-info-grid span{display:block;margin-top:4px;color:#f8fafc;word-break:break-word}@media(max-width:760px){.pfsp-v14-info-grid{grid-template-columns:1fr}}'; document.head.appendChild(st);
  }
  ...
  function renderResult(payload, uploadHash=''){
    state.lastPayload = payload; if(payload?.certificate) state.lastCertificate = payload.certificate; ensureInfoStyles();
    ...
    box.innerHTML = `<div class="pfsp-v11-verdict">${html(label(payload?.verdict))}</div><div class="pfsp-v11-message">${html(payload?.message || '')}</div>${extras}${renderDocumentInformation(r)}`;
