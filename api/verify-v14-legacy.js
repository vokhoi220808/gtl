// PDF Fusion Smart Pro - Verify System v14 Enterprise Trust Suite + Document Information
...
const API_VERSION = '14.4.0-enterprise-trust-suite';
const REGISTRY_VERSION = 'PFSP-VERIFY-REGISTRY-v5';
const LEGACY_REGISTRY_VERSIONS = ['PFSP-VERIFY-REGISTRY-v4', 'PFSP-VERIFY-REGISTRY-v3'];
const SIGNATURE_VERSION = 'PFSP-SERVER-SIGNED-CERT-v5';
const LEGACY_SIGNATURE_VERSIONS = ['PFSP-SERVER-SIGNED-CERT-v4', 'PFSP-SERVER-SIGNED-CERT-v3'];
const SIGNATURE_ALGORITHM = 'AUTO';
const DEFAULT_KEY_ID = cleanString(process.env.PFSP_VERIFY_KEY_ID || process.env.VERIFY_KEY_ID || 'pfsp-v14-default', 80);
...
const MAX_DOCUMENT_INFO_JSON = Number(process.env.PFSP_VERIFY_MAX_DOCUMENT_INFO_JSON || 24000);
...
function normalizeCustomFields(value) {
  ...
  else if (value && typeof value === 'object') arr = Object.entries(value).map(([label, val]) => ({ label, value: val }));
  return arr.map(item => ({
...
function compactObject(obj) {
  ...
  for (const [k, v] of Object.entries(obj || {})) {
    ...
    if (v && typeof v === 'object') { const inner = compactObject(v); if (Object.keys(inner).length) out[k] = inner; continue; }
    if (v !== '' && v != null) out[k] = v;
  }
  return out;
}
...
function publicInfo(record = {}) {
  return {
    documentInfo: normalizeDocumentInfo(record.documentInfo || {}),
    userInfo: normalizeUserInfo(record.userInfo || record.personInfo || {}),
    extraInfo: normalizeExtraInfo(record.extraInfo || {})
  };
}
function v14KeyConfig() {
  let keys = [];
  try {
  ...
  return keys.map(k => ({
    id: cleanString(k.id || k.keyId || DEFAULT_KEY_ID, 80),
    privateKey: String(k.privateKey || '').replace(/\\n/g, '\n'),
    publicKey: String(k.publicKey || '').replace(/\\n/g, '\n'),
    algorithm: cleanString(k.algorithm || 'RSA-SHA256', 40),
    active: k.active !== false,
    revoked: !!k.revoked
  })).filter(k => k.id);
}
function activeSigningKey() {
  return v14KeyConfig().find(k => k.active && !k.revoked && k.privateKey) || null;
}
function findVerifyKey(keyId) {
  const keys = v14KeyConfig();
  return keys.find(k => k.id === keyId && !k.revoked && (k.publicKey || k.privateKey)) || keys.find(k => !k.revoked && (k.publicKey || k.privateKey)) || null;
}
function signWithAsymmetricKey(key, text) {
  const sign = crypto.createSign(key.algorithm || 'RSA-SHA256');
  sign.update(String(text || ''), 'utf8');
  sign.end();
  return sign.sign(key.privateKey, 'base64url');
}
function verifyWithAsymmetricKey(key, text, signature) {
  const verify = crypto.createVerify(key.algorithm || 'RSA-SHA256');
  verify.update(String(text || ''), 'utf8');
  verify.end();
  return verify.verify(key.publicKey || key.privateKey, String(signature || ''), 'base64url');
}
...
function signRecord(record) {
  const payload = SIGNATURE_VERSION + '|' + canonicalRecord(record);
  const key = activeSigningKey();
  if (key) return signWithAsymmetricKey(key, payload);
  const secret = signingSecret();
  if (!secret) return '';
  return hmacText(secret, payload);
}
function signatureAlgorithmForRecord(record) {
  return cleanString(record.signatureAlgorithm || (record.keyId ? 'RSA-SHA256' : 'HMAC-SHA256'), 80);
}
function expectedSignatures(record) {
  ...
  if (secret) {
    for (const version of versions) {
      for (const canonical of canonicals) pairs.push({ version, algorithm: 'HMAC-SHA256', signature: hmacText(secret, version + '|' + canonical) });
    }
  }
  return pairs;
}
function verifyRecordSignature(record) {
  ...
  const keyId = cleanString(record.keyId || '', 80);
  const algorithm = signatureAlgorithmForRecord(record);
  if (/RSA|ECDSA|ED25519/i.test(algorithm) || keyId) {
    const key = findVerifyKey(keyId);
    if (!key) return { present: true, valid: null, reason: 'public-key-not-configured', algorithm, keyId };
    const versions = [...new Set([record.signatureVersion, SIGNATURE_VERSION, ...LEGACY_SIGNATURE_VERSIONS].filter(Boolean))];
    const canonicals = [canonicalRecord(record), legacyCanonicalRecord(record)];
    for (const version of versions) {
      for (const canonical of canonicals) {
        try {
          if (verifyWithAsymmetricKey(key, version + '|' + canonical, record.serverSignature)) {
            return { present: true, valid: true, algorithm: key.algorithm || algorithm, version, keyId: key.id, compatibility: version === SIGNATURE_VERSION ? 'current' : 'legacy-compatible' };
          }
        } catch (err) {}
      }
    }
    return { present: true, valid: false, reason: 'signature-mismatch', algorithm: key.algorithm || algorithm, keyId: key.id };
  }
  const candidates = expectedSignatures(record);
  if (!candidates.length) return { present: true, valid: null, reason: 'signing-secret-or-public-key-not-configured', algorithm };
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (actual.length === expected.length && crypto.timingSafeEqual(actual, expected)) {
        return { present: true, valid: true, algorithm: candidate.algorithm, version: candidate.version, compatibility: candidate.version === SIGNATURE_VERSION ? 'current' : 'legacy-compatible' };
      }
    }
    return { present: true, valid: false, reason: 'signature-mismatch', algorithm, version: record.signatureVersion || SIGNATURE_VERSION };
  } catch (err) {
    return { present: true, valid: false, reason: 'bad-signature-format', algorithm };
  }
}
function requireSigning(req, res) {
  if (signingSecret() || activeSigningKey()) return true;
  send(req, res, 503, { ok: false, verdict: 'SIGNING_NOT_CONFIGURED', level: 'bad', message: 'PFSP_VERIFY_SIGNING_SECRET or PFSP_VERIFY_PRIVATE_KEY is required before records can be registered or modified.' });
  return false;
}
...
function emptyRegistry() {
  return {
    ...
    settings: {
      ...
      schema: 'v5',
      enterpriseSigningSupported: true,
      keyRotationSupported: true,
      adapterStorageSupported: true,
      analyticsSupported: true,
      bulkActionsSupported: true,
      documentInformationSupported: true
    },
    records: [],
    auditLog: [],
    snapshots: [],
...
function normalizeRecord(record) {
  ...
  r.documentInfo = normalizeDocumentInfo(r.documentInfo || { title: r.documentTitle || '', description: r.note || '', documentType: r.documentType || '', issuer: r.issuer || '', keywords: r.tags || [] });
  r.userInfo = normalizeUserInfo(r.userInfo || r.personInfo || { fullName: r.owner || '', organization: r.organization || '', role: r.role || '' });
...
function normalizeRegistry(input) {
  ...
  base.schemaVersion = Number(src.schemaVersion || src.schema || 5) || 5;
  base.updatedAt = safeDate(src.updatedAt) || src.updatedAt || base.updatedAt;
  base.settings = { ...base.settings, ...(src.settings || {}), schema: 'v5' };
...
function registryIntegrity(registry) {
  ...
  const digestPayload = {
    ...
    schemaVersion: registry.schemaVersion || 5,
...
async function readRegistryFromGithub() {
  ...
  const meta = await fetchJson(api, {
    ...
    'User-Agent': 'pdf-fusion-smart-pro-verify-v14'
  });
...
function redisConfig() {
  return {
    url: process.env.PFSP_UPSTASH_REDIS_REST_URL || process.env.UPSTASH_REDIS_REST_URL || '',
    token: process.env.PFSP_UPSTASH_REDIS_REST_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN || '',
    key: process.env.PFSP_REDIS_REGISTRY_KEY || 'pfsp:verify:registry:v5'
  };
}
async function readRegistryFromRedis() {
  const cfg = redisConfig();
  if (!cfg.url || !cfg.token) return null;
  const url = cfg.url.replace(/\/$/, '') + '/get/' + encodeURIComponent(cfg.key);
  const payload = await fetchJson(url, { Authorization: `Bearer ${cfg.token}` });
...
async function writeRegistryToRedis(registry) {
  const cfg = redisConfig();
  if (!cfg.url || !cfg.token) return null;
  ...
  const url = cfg.url.replace(/\/$/, '') + '/set/' + encodeURIComponent(cfg.key);
  ...
  if (!r.ok || json.error) throw new Error(json.error || `Redis write failed ${r.status}`);
  return { redis: true, key: cfg.key };
}
async function readRegistry() {
  ...
  try {
    const redis = await readRegistryFromRedis();
...
async function persistRegistry(registry, message) {
  ...
  registry.settings = { ...emptyRegistry().settings, ...(registry.settings || {}), schema: 'v5' };
  ...
  try {
    const redis = await writeRegistryToRedis(registry);
    if (redis) return redis;
  } catch (err) {}
...
function envSummary() {
  ...
  return {
    ...
    signatureMode: activeSigningKey() ? 'asymmetric-key' : signingSecret() ? 'hmac-secret' : 'not-configured',
    activeKeyId: activeSigningKey()?.id || '',
    configuredVerifyKeys: keys.length,
    redisConfigured: !!(redis.url && redis.token),
    redisRegistryKey: redis.key,
    storageBackend: redis.url && redis.token ? 'redis-primary-github-backup' : cfg.token && cfg.owner && cfg.repo ? 'github-json' : process.env.PFSP_REGISTRY_URL ? 'remote-json-readonly' : 'local-json-readonly',
    ...
    signingConfigured: !!(signingSecret() || activeSigningKey()),
...
function makeRecord(input, req, mode = 'manual') {
  ...
  const record = normalizeRecord({
    ...
    owner: input.owner || cert.owner || cert.userInfo?.fullName || cert.personInfo?.fullName || cert.userInfo?.organization || '',
    project: input.project || cert.project || cert.extraInfo?.project || cert.documentInfo?.department || '',
    documentTitle: input.documentTitle || cert.documentTitle || cert.title || cert.documentInfo?.title || '',
    ...
    documentInfo: cert.documentInfo || input.documentInfo || {},
    userInfo: cert.userInfo || cert.personInfo || input.userInfo || input.personInfo || {},
    extraInfo: cert.extraInfo || input.extraInfo || {},
    signatureVersion: SIGNATURE_VERSION,
    keyId: activeSigningKey()?.id || '',
    signatureAlgorithm: activeSigningKey()?.algorithm || 'HMAC-SHA256'
  });
  ...
  record.signatureAlgorithm = activeSigningKey()?.algorithm || 'HMAC-SHA256';
  record.keyId = activeSigningKey()?.id || record.keyId || '';
  if (mode === 'auto') {
...
function signedCertificate(record) {
  return {
    certificateVersion: 'PFSP-TRUSTED-CERT-v5',
    app: record.app,
    ...
    keyId: record.keyId || '',
    signatureAlgorithm: record.signatureAlgorithm || (record.keyId ? 'RSA-SHA256' : 'HMAC-SHA256'),
...
function publicPortalRecord(record, req) {
  ...
  return {
    ...
    size: record.size,
    fileName: record.fileName,
    documentTitle: record.documentTitle || record.documentInfo?.title || record.fileName,
    documentInfo: normalizeDocumentInfo(record.documentInfo || {}),
    userInfo: normalizeUserInfo(record.userInfo || record.personInfo || {}),
    extraInfo: normalizeExtraInfo(record.extraInfo || {}),
    owner: record.owner || '',
...
async function handleTrustPortal(req, res, input = {}) {
  ...
  return send(req, res, 200, {
    ...
    portalVersion: '14.4.0-enterprise-trust-suite',
...
async function handlePrintableCertificate(req, res, input = {}) {
  ...
  return send(req, res, 200, {
    ...
    trust,
    registryIntegrity: registryIntegrity(registry),
    portal: { version: '14.4.0-enterprise-trust-suite', certificatePage: (getBaseUrl(req, record) || '.') + '/verify-certificate.html?id=' + encodeURIComponent(record.id) }
  });
}
...
function listMatches(record, q, status) {
  ...
  if (!q) return true;
  const hay = [record.id, record.sha256, record.fileName, record.origin, record.status, record.note, record.owner, record.project, record.documentTitle, stableStringify(publicInfo(record)), ...(record.tags || [])].join(' ').toUpperCase();
  return hay.includes(q);
}
...
function analyticsSummary(registry) {
  ...
  const byStatus = summarizeStatus(records);
  const top = (getter, limit = 12) => {
    const map = new Map();
    for (const r of records) {
      const values = getter(r);
      for (const value of (Array.isArray(values) ? values : [values])) {
        const k = cleanString(value, 160);
  ...
  for (const item of audit) {
    const day = cleanString(item.time, 10);
    if (!day) continue;
    const row = days.get(day) || { date: day, total: 0, verify: 0, register: 0, admin: 0 };
    row.total += 1;
    if (/verify|check/i.test(item.action || '')) row.verify += 1;
    if (/register/i.test(item.action || '')) row.register += 1;
    if (/revoke|restore|suspend|activate|repair|metadata|bulk/i.test(item.action || '')) row.admin += 1;
    days.set(day, row);
  }
  return {
    byStatus,
    topOwners: top(r => r.owner || r.userInfo?.fullName || ''),
    topProjects: top(r => r.project || ''),
    topTags: top(r => r.tags || []),
    topDocumentTypes: top(r => r.documentInfo?.documentType || ''),
    timeline: [...days.values()].sort((a, b) => String(b.date).localeCompare(String(a.date))).slice(0, 45),
    generatedAt: nowIso()
  };
}
async function handleAnalytics(req, res, body) {
  if (!hasAdmin(req, body)) return send(req, res, 401, { ok: false, verdict: 'UNAUTHORIZED', level: 'bad', message: 'Missing or invalid admin secret.' });
  const registry = await readRegistry();
...
async function handleBulkAction(req, res, body) {
  if (!hasAdmin(req, body)) return send(req, res, 401, { ok: false, verdict: 'UNAUTHORIZED', level: 'bad', message: 'Missing or invalid admin secret.' });
  if (!requireSigning(req, res)) return;
  const ids = Array.isArray(body.ids) ? body.ids.map(x => cleanString(x, 120).toUpperCase()).filter(Boolean).slice(0, MAX_BULK_REGISTER) : [];
  const op = lower(body.operation || body.bulkOperation || '', 40);
  if (!ids.length) return send(req, res, 400, { ok: false, verdict: 'NO_IDS', level: 'bad', message: 'Provide ids[] for bulk action.' });
  if (!['revoke','suspend','activate','restore','archive'].includes(op)) return send(req, res, 400, { ok: false, verdict: 'BAD_BULK_OPERATION', level: 'bad', message: 'Supported operations: revoke, suspend, activate, restore, archive.' });
  const registry = await readRegistry();
  const results = [];
  for (const id of ids) {
    const record = findRecord(registry, id);
    if (!record) { results.push({ id, ok: false, verdict: 'NOT_FOUND' }); continue; }
    if (op === 'revoke') { record.status = 'revoked'; record.revokedAt = nowIso(); record.revokeReason = cleanString(body.reason || 'bulk revoke', 300); }
    if (op === 'suspend') { record.status = 'suspended'; record.suspendedAt = nowIso(); record.suspendReason = cleanString(body.reason || 'bulk suspend', 300); }
    if (op === 'activate' || op === 'restore') { record.status = 'active'; record.restoredAt = nowIso(); record.revokedAt = ''; record.suspendedAt = ''; record.revokeReason = ''; record.suspendReason = ''; }
    if (op === 'archive') { record.status = 'archived'; }
    record.updatedAt = nowIso();
    pushHistory(record, 'bulk-' + op, { actor: 'admin', result: op.toUpperCase(), message: body.reason || '', sha256: record.sha256, status: record.status, origin: record.origin });
    record.serverSignature = signRecord(record);
    record.certificateSignature = record.serverSignature;
    record.signatureVersion = SIGNATURE_VERSION;
    record.signatureAlgorithm = activeSigningKey()?.algorithm || 'HMAC-SHA256';
    record.keyId = activeSigningKey()?.id || record.keyId || '';
    addAudit(registry, req, { action: 'bulk-' + op, id, sha256: record.sha256, result: op.toUpperCase(), ok: true, message: body.reason || '', actor: 'admin', origin: record.origin });
    results.push({ id, ok: true, verdict: op.toUpperCase(), record: publicRecord(record, { includeHistory: true, admin: true }) });
  }
  if (body.dryRun) return send(req, res, 200, { ok: true, verdict: 'BULK_ACTION_DRY_RUN', level: 'good', message: 'Bulk action validated. No write was made.', results, registryIntegrity: registryIntegrity(registry) });
  try {
    const saved = await persistRegistry(registry, `Bulk ${op} ${results.filter(r => r.ok).length} PFSP verify records`);
    return send(req, res, 200, { ok: true, verdict: 'BULK_ACTION_DONE', level: 'good', message: `Bulk ${op} completed.`, results, commit: saved.commit && saved.commit.html_url, localWrite: saved.local || false, redisWrite: saved.redis || false, registryIntegrity: registryIntegrity(registry) });
  } catch (err) {
    return send(req, res, 202, { ok: false, verdict: 'BULK_ACTION_MANUAL_PATCH_REQUIRED', level: 'warn', message: 'Bulk action prepared but durable write failed.', error: err.message, results, suggestedRegistry: registry, registryIntegrity: registryIntegrity(registry) });
  }
}
async function handleSelfTest(req, res) {
  ...
  const checks = [
    ...
    { name: 'signingSecret', ok: env.signingConfigured, message: env.signingConfigured ? 'Signing configured via ' + env.signatureMode + '.' : 'Missing PFSP_VERIFY_SIGNING_SECRET or PFSP_VERIFY_PRIVATE_KEY.' },
    ...
    { name: 'enterpriseSigning', ok: env.signatureMode !== 'not-configured', message: 'Signature mode: ' + env.signatureMode + (env.activeKeyId ? ' · key ' + env.activeKeyId : '') + '.' },
    { name: 'storageAdapter', ok: !!env.storageBackend, message: 'Storage backend: ' + env.storageBackend + '.' },
    { name: 'documentInformation', ok: true, message: 'Document Information fields are supported and signed with the record.' }
  ];
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const adminActions = ['auto-register','autoregister','register','bulk-register','register-bulk','revoke','restore','suspend','hold','activate','unsuspend','expiry','update-expiry','set-expiry','update-note','note','metadata','repair','resign','list','records','search','audit','audit-log','backup','snapshot','export','analytics','bulk-action','bulk-status'];
      ...
      if (['update-note', 'note', 'metadata'].includes(action)) return handleRecordPatch(req, res, body, record => {
        ...
        if ('documentInfo' in body) record.documentInfo = normalizeDocumentInfo(body.documentInfo);
        if ('userInfo' in body || 'personInfo' in body) record.userInfo = normalizeUserInfo(body.userInfo || body.personInfo);
        if ('extraInfo' in body) record.extraInfo = normalizeExtraInfo(body.extraInfo);
      ...
      if (action === 'analytics') return handleAnalytics(req, res, body);
