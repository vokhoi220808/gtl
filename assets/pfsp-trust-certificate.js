/* PDF Fusion Trust Certificate printable page v14 */
(function(){
  'use strict';
  ...
  function infoCard(record){ const rows=infoRows(record); return rows.length?`<section class="pfsp-v11-card"><h2>Document Information / Thông tin tài liệu</h2><div class="pfsp-v14-info-grid">${rows.map(([k,v])=>`<div><b>${esc(k)}</b><span>${esc(v)}</span></div>`).join('')}</div></section>`:''; }
  function render(payload){
    ...
    box.innerHTML=`
      <section class="pfsp-v11-hero pfsp-v14-print-card">
        <div class="pfsp-v11-top"><div class="pfsp-v11-brand"><div class="pfsp-v11-logo">🛡️</div><div><div class="pfsp-v11-kicker">PDF Fusion Smart Pro · Trust Certificate v14</div><h1>Certificate xác minh tài liệu</h1><p>Chứng nhận metadata công khai từ server-signed trusted registry. File PDF gốc không được lưu trên server.</p></div></div><div class="pfsp-v14-print-actions"><button class="pfsp-v11-btn primary" onclick="window.print()">In / Lưu PDF</button><a class="pfsp-v11-btn" href="./verify.html?id=${encodeURIComponent(r.id)}">Mở verify</a></div></div>
        <div class="pfsp-v14-cert-verdict"><div><span>Trust Score</span><b>${esc(trust.trustScore ?? '-')}</b><small>/100 · ${esc(trust.trustLevel || '')}</small></div><div>${badge(r.status)}<p>${esc(payload.signature?.valid===true?'Server signature hợp lệ':payload.signature?.valid===false?'Chữ ký registry không hợp lệ':'Chưa xác nhận chữ ký')}</p></div></div>
      </section>
