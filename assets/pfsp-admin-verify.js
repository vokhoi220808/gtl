/* PDF Fusion Smart Pro - Verify Admin Console v14 Enterprise Trust Suite */
(function(){
  'use strict';
  ...
  function jsonField(id){const raw=String($(id)?.value||'').trim(); if(!raw) return {}; try{return JSON.parse(raw)}catch(err){throw new Error(id + ' JSON lỗi: ' + err.message)}}
  ...
  function registrationPayload(dryRun=false){
    const documentInfo = jsonField('regDocumentInfo');
    const userInfo = jsonField('regUserInfo');
    if($('regTitle')?.value.trim() && !documentInfo.title) documentInfo.title = $('regTitle').value.trim();
    if($('regOwner')?.value.trim() && !userInfo.fullName) userInfo.fullName = $('regOwner').value.trim();
    return { action:'register', id:$('regId')?.value.trim() || '', sha256:normalizeHash($('regHash')?.value), fileName:$('regName')?.value.trim() || 'document.pdf', size:$('regSize')?.value.trim(), origin:$('regOrigin')?.value.trim() || location.origin, owner:$('regOwner')?.value.trim() || userInfo.fullName || userInfo.organization || '', project:$('regProject')?.value.trim() || documentInfo.department || '', documentTitle:$('regTitle')?.value.trim() || documentInfo.title || '', documentInfo, userInfo, tags:tags($('regTags')?.value), expiresAt:isoFromLocal($('regExpiry')?.value), note:$('regNote')?.value.trim() || '', overwrite:!!$('regOverwrite')?.checked, dryRun };
  }
  ...
  async function updateMeta(){
    let documentInfo={}, userInfo={};
    try{ documentInfo=jsonField('metaDocumentInfo'); userInfo=jsonField('metaUserInfo'); }catch(err){ render({ok:false, verdict:'BAD_JSON', level:'bad', message:err.message}); return; }
    const payload = await post({action:'update-note', id:$('metaId')?.value.trim(), owner:$('metaOwner')?.value.trim(), project:$('metaProject')?.value.trim(), documentTitle:$('metaTitle')?.value.trim(), tags:tags($('metaTags')?.value), note:$('metaNote')?.value.trim(), documentInfo, userInfo});
  ...
  function renderAnalytics(payload){
    ...
    const list=rows=> (rows||[]).map(x=>`<span class="pfsp-v11-badge warn">${html(x.name)} · ${html(x.count)}</span>`).join(' ') || '<span class="pfsp-v11-small">Không có dữ liệu.</span>';
    box.className='pfsp-v11-result '+cls(payload);
    box.innerHTML=`<div class="pfsp-v11-verdict">${html(label(payload.verdict))}</div><div class="pfsp-v11-kv"><b>Total</b><code>${html(a.byStatus?.total??'-')}</code><b>Active</b><code>${html(a.byStatus?.active??'-')}</code><b>Revoked</b><code>${html(a.byStatus?.revoked??'-')}</code><b>Expired</b><code>${html(a.byStatus?.expired??'-')}</code></div><h3>Top owners</h3>${list(a.topOwners)}<h3>Top projects</h3>${list(a.topProjects)}<h3>Top document types</h3>${list(a.topDocumentTypes)}<h3>Top tags</h3>${list(a.topTags)}<textarea class="pfsp-v11-textarea" readonly>${html(JSON.stringify(a.timeline||[],null,2))}</textarea>`;
  }
  async function loadAnalytics(){ renderAnalytics(await post({action:'analytics'})); }
  function bulkActionPayload(dryRun=false){ return {action:'bulk-action', operation:$('bulkActionOp')?.value||'suspend', ids:String($('bulkActionIds')?.value||'').split(/\n+/).map(x=>x.trim()).filter(Boolean), reason:$('bulkActionReason')?.value.trim()||'', dryRun}; }
  async function bulkAction(dryRun=false){ const payload=await post(bulkActionPayload(dryRun)); render(payload); if(payload.ok&&!dryRun) loadRecords(); }
  function bind(){
    ...
    $('analytics') && ($('analytics').onclick = loadAnalytics);
    $('loadAnalytics') && ($('loadAnalytics').onclick = loadAnalytics);
    ...
    $('bulkActionDry') && ($('bulkActionDry').onclick = ()=>bulkAction(true));
    $('bulkActionWrite') && ($('bulkActionWrite').onclick = ()=>bulkAction(false));
  }
