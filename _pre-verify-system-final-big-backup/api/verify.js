// PDF Fusion Smart Pro - Trusted Verify Registry API v9.0.0 Auto Register
// Vercel Serverless Function. No external npm dependencies.
// Public GET checks whether an ID/hash is registered as genuine.
// POST register/revoke requires VERIFY_ADMIN_SECRET.
// POST auto-register is intentionally public for QR export automation; it never revokes
// and never overwrites an existing different hash. GitHub repo env vars provide durable writes.
// Set PFSP_AUTO_REGISTER_ENABLED=false to disable public auto-registration.
...
function autoRegisterEnabled() {
  const value = String(process.env.PFSP_AUTO_REGISTER_ENABLED || process.env.PFSP_VERIFY_AUTO_REGISTER || 'true').toLowerCase();
  return !['0', 'false', 'no', 'off', 'disabled'].includes(value);
}

function clientIp(req) {
  return cleanString((req.headers['x-forwarded-for'] || '').split(',')[0] || req.socket?.remoteAddress || '', 80);
}

function isSameHash(a, b) {
  return normalizeHash(a) && normalizeHash(a) === normalizeHash(b);
}
...
async function handleAutoRegister(req, res, body) {
  if (!autoRegisterEnabled()) return send(res, 403, { ok: false, verdict: 'AUTO_REGISTER_DISABLED', message: 'Public QR auto-registration is disabled on this deployment.' });
  const record = makeRecord({
    ...body,
    registeredBy: cleanString(body.registeredBy || 'auto-qr-export', 120),
    note: cleanString(body.note || 'Automatically registered when a QR Verify PDF was exported.', 500)
  });
  record.registrationMode = 'auto-qr-export';
  record.clientIpHash = clientIp(req) ? sha256Text(clientIp(req)).slice(0, 24).toUpperCase() : '';
  record.userAgentHash = req.headers['user-agent'] ? sha256Text(req.headers['user-agent']).slice(0, 24).toUpperCase() : '';
  record.serverSignature = signRecord(record);
  ...
  if (idx >= 0) {
    ...
    if (isSameHash(existing.sha256, record.sha256)) {
      return send(res, 200, { ok: true, verdict: 'ALREADY_REGISTERED', message: 'This QR Verify PDF was already registered with the same SHA-256 hash.', record: existing });
    }
  ...
  try {
    const gh = await writeRegistryToGithub(registry, `Auto-register PFSP QR Verify ID ${record.id}`);
    return send(res, 200, { ok: true, verdict: 'AUTO_REGISTERED', message: 'QR Verify PDF was automatically registered in the trusted registry.', record, commit: gh.commit && gh.commit.html_url });
  } catch (err) {
    return send(res, 202, {
      ok: false,
      verdict: 'AUTO_REGISTER_PENDING_MANUAL_PATCH',
      message: 'Auto-registration created a valid record, but durable GitHub write is not configured or failed. Commit the suggested registry JSON manually.',
      error: err.message,
      record,
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (url.searchParams.get('health')) return send(res, 200, { ok: true, version: '9.0.0-auto-register-verify-system', autoRegister: autoRegisterEnabled(), time: new Date().toISOString() });
      const registry = await readRegistry();
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'auto-register' || action === 'autoregister') return handleAutoRegister(req, res, body);
