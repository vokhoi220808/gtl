/* PDF Fusion Smart Pro - Document Hash Verify Upgrade v7.0.0
...
(function(){
  ...
  async function buildCertificate(blob, name, type){
    ...
    return {
      version:'PFSP-DOC-HASH-v1',
      app:'PDF Fusion Smart Pro',
      appVersion:'7.0.0-document-hash-verify',
  ...
  function injectPanel(){
    ...
    ['qrDocHashEnable','qrHashSourceEnable','qrHashAutoCert'].forEach(id=>$(id)?.addEventListener('change',()=>{saveSettings(); try{ if(typeof updateQrUi === 'function') updateQrUi(); }catch{};}));
  ...
  function injectStyles(){
    ...
    const st=document.createElement('style'); st.id='pfspDocHashStyle'; st.textContent=`
      .pfsp-doc-hash-panel{margin-top:12px;border:1px solid rgba(34,211,163,.28);border-radius:18px;padding:14px;background:linear-gradient(135deg,rgba(34,211,163,.10),rgba(139,124,255,.10));box-shadow:0 18px 45px rgba(0,0,0,.18)}
      .pfsp-hash-head{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-bottom:8px}.pfsp-hash-head strong{color:#fff}.pfsp-hash-head span{font-size:12px;color:var(--muted)}
      .pfsp-hash-check{display:block;margin:7px 0;color:var(--text)}.pfsp-hash-line,.pfsp-hash-last,.pfsp-hash-status{margin-top:8px;border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:10px;background:rgba(0,0,0,.16);font-size:12px;color:var(--muted);word-break:break-all}.pfsp-hash-line code,.pfsp-hash-last code{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;color:#b8fff0}.pfsp-hash-status[data-kind="good"]{border-color:rgba(34,211,163,.42);color:#b8fff0}.pfsp-hash-status[data-kind="warn"]{border-color:rgba(255,209,102,.45);color:#ffe2a3}.pfsp-hash-status[data-kind="bad"]{border-color:rgba(255,107,136,.45);color:#ffb5c2}`;
  ...
  window[NS] = {state, sha256, hashChecksum, prepareSourceHash, qrQuery, captureOutput, downloadCertificate, copyVerifyLink, clearLast, getLastCertificate};
