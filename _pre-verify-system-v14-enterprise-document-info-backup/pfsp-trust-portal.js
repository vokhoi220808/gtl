/* PDF Fusion Smart Pro - Trust Portal v13 */
(function(){
  'use strict';
  ...
  function renderResult(boxId, payload){
    ...
    box.innerHTML = `<div class="pfsp-v11-verdict">${esc(payload.verdict || 'RESULT')}</div><div class="pfsp-v11-message">${esc(payload.message || '')}</div>` +
      (typeof payload.trustScore !== 'undefined' ? `<div class="pfsp-v13-score-line"><b>${esc(payload.trustScore)}</b><span>/100 · ${esc(payload.trustLevel || 'unknown')}</span></div>` : '') +
