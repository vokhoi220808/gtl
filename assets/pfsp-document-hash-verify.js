...
(function(){
  ...
  function collectDocumentInformation(){
    try{
      if(typeof window.PFSPCollectDocumentInformation === 'function') return window.PFSPCollectDocumentInformation() || {};
    }catch{}
    const get=id=>String($(id)?.value||'').trim();
    const tags=v=>String(v||'').split(',').map(x=>x.trim()).filter(Boolean);
    const compact=obj=>{
      if(Array.isArray(obj)) return obj.map(compact).filter(x=>x!==''&&x!=null&&!(typeof x==='object'&&!Object.keys(x).length));
      if(obj&&typeof obj==='object'){
        const out={}; Object.entries(obj).forEach(([k,v])=>{const c=compact(v); if(!(c===''||c==null||(Array.isArray(c)&&!c.length)||(typeof c==='object'&&!Array.isArray(c)&&!Object.keys(c).length))) out[k]=c;}); return out;
      }
      return obj;
    };
    let custom={};
    try{ custom=get('docInfoCustomJson')?JSON.parse(get('docInfoCustomJson')):{}; }catch(err){ custom={customJsonError:err.message, raw:get('docInfoCustomJson')}; }
    return compact({
      documentInfo:{ title:get('docInfoTitle'), documentType:get('docInfoType'), documentNumber:get('docInfoNumber'), version:get('docInfoVersion'), language:get('docInfoLanguage'), issuer:get('docInfoIssuer'), department:get('docInfoDepartment'), author:get('docInfoAuthor'), subject:get('docInfoSubject'), confidentiality:get('docInfoConfidentiality'), description:get('docInfoDescription'), createdDate:get('docInfoCreatedDate'), effectiveDate:get('docInfoEffectiveDate'), expiresDate:get('docInfoExpiresDate'), keywords:tags(get('docInfoKeywords')) },
      userInfo:{ fullName:get('docInfoOwnerName'), email:get('docInfoOwnerEmail'), phone:get('docInfoOwnerPhone'), organization:get('docInfoOwnerOrganization'), department:get('docInfoOwnerDepartment'), role:get('docInfoOwnerRole'), website:get('docInfoOwnerWebsite'), identifier:get('docInfoOwnerIdentifier'), address:get('docInfoOwnerAddress'), note:get('docInfoOwnerNote') },
      extraInfo: custom
    });
  }
  async function buildCertificate(blob, name, type){
    ...
    const documentBundle = collectDocumentInformation();
    return {
      version:'PFSP-DOC-HASH-v2',
      app:'PDF Fusion Smart Pro',
      appVersion:'14.4.0-enterprise-trust-suite',
      ...
      checksum:hck,
      verifyUrl,
      documentInfo: documentBundle.documentInfo || {},
      userInfo: documentBundle.userInfo || {},
      personInfo: documentBundle.userInfo || {},
      extraInfo: documentBundle.extraInfo || {},
      owner: documentBundle.userInfo?.fullName || documentBundle.userInfo?.organization || '',
      project: documentBundle.extraInfo?.project || documentBundle.documentInfo?.department || '',
      documentTitle: documentBundle.documentInfo?.title || '',
  ...
  function injectPanel(){
    ...
    div.innerHTML=`
      <div class="pfsp-hash-head">
        <strong>🔐 Document Hash Verify <span class="tag">SHA-256 + Info</span></strong>
        <span>So hash PDF và gắn Document Information vào Verify ID</span>
      </div>
